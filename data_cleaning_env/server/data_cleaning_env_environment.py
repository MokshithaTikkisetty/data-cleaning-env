...
def grade(submitted: list, expected: list) -> tuple[float, str]:
    if not submitted:
        return 0.01, "No data submitted."
    submitted = submitted[:len(expected)]
    if len(submitted) != len(expected):
        return 0.10, f"Expected {len(expected)} rows, got {len(submitted)}."
    total_fields = 0
    correct_fields = 0
    feedback_lines = []
    for i, (sub_row, exp_row) in enumerate(zip(submitted, expected)):
        for key, exp_val in exp_row.items():
            ...
            if str(sub_val).strip().lower() == str(exp_val).strip().lower():
                ...
                feedback_lines.append(f"Row {i+1} field '{key}': expected '{exp_val}', got '{sub_val}'")
    score = round(correct_fields / total_fields, 2) if total_fields > 0 else 0.01
    score = max(0.01, min(0.99, score))
    feedback = "Almost perfect!" if score >= 0.99 else "Issues: " + "; ".join(feedback_lines[:3])
...
class DataCleaningEnvironment(Environment):
    ...
    def step(self, action: DataCleaningAction) -> DataCleaningObservation:
        ...
        done = score >= 0.99 or self._state.step_count >= 3
        return DataCleaningObservation(
